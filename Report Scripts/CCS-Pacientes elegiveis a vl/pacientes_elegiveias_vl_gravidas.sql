				...
				(  SELECT   patient_id , max(data_ult_carga)  data_ult_carga , valor_cv
                    ...
                    (  
						   SELECT 	e.patient_id,
							...
							FROM 	encounter e
									inner join obs o on e.encounter_id=o.encounter_id
							where 	e.encounter_type in (13,6,9) and e.voided=0 and encounter_datetime between date_sub(:endDate, interval 6 MONTH) and  :endDate  
				    ...
				    UNION ALL
							...
							FROM 	encounter e
									inner join obs o on e.encounter_id=o.encounter_id
							where 	e.encounter_type = 53 and e.voided=0 and o.voided=0 and obs_datetime between date_sub(:endDate, interval 6 MONTH) and  :endDate  
...
/****************************************************************************/
	WHERE
          ...
          cv_qualitativa.carga_viral_qualitativa is null and (
          cv.patient_id is null or
          /*   4. Resultado de CV>1000copias/ml  há mais de 3 meses */
           (cv.value_numeric > 1000 and DATEDIFF(:endDate,cv.data_ult_cv)/30 >3 ) 
           )
