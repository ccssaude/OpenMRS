...
WHERE it.datainiciotarv <= "',TRIM(end_date),'" and it.patient_id not in (select distinct patient_id from openmrs_reports.estadopaciente17 WHERE estado != "ACTIVO" and dataestado <= "',TRIM(end_date),'")');
                PREPARE stmt FROM @SQL;
		EXECUTE stmt;
		DEALLOCATE PREPARE stmt;	
