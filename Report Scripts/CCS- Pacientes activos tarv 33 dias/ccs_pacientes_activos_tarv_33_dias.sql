...
(SELECT 	inicio_real.patient_id,
	...
	(	SELECT patient_id,MIN(data_inicio) data_inicio
       ...
       /************************** keypop concept_id = 23703 ****************************/
        ...
        /************  Peso  *********************/
        LEFT JOIN(  
        	SELECT 	e.patient_id,o.value_numeric peso,  MAX(encounter_datetime)
			FROM	 encounter e 
					INNER JOIN obs o ON o.encounter_id=e.encounter_id
			WHERE 	e.voided=0 AND o.voided=0 AND o.concept_id=5089 AND e.encounter_type IN (6,9,53)  
                    AND e.form_id = 163 
			GROUP BY e.patient_id
               ...
               /************  Altura  *********************/
        LEFT JOIN(  
        	SELECT 	e.patient_id,o.value_numeric altura,  MAX(encounter_datetime)
			FROM	 encounter e
					INNER JOIN obs o ON o.encounter_id=e.encounter_id
			...
			GROUP BY e.patient_id
                       ...
                       /************  Hemoglobina  *********************/
        LEFT JOIN(  
        	SELECT 	e.patient_id,o.value_numeric hemoglobina,  MAX(encounter_datetime)
			FROM	 encounter e 
					INNER JOIN obs o ON o.encounter_id=e.encounter_id
			WHERE 	e.voided=0  AND 
...
SELECT 	e.patient_id,
        ...
        /****************** ****************************  CD4   ********* *****************************************************/
        LEFT JOIN(  
               	SELECT 	cd4.patient_id, cd4.value_numeric cd4,  MAX(cd4.encounter_datetime)
            FROM ( select e.patient_id, o.value_numeric , encounter_datetime
					...
					FROM encounter e 
							INNER JOIN obs o ON o.encounter_id=e.encounter_id
					WHERE 	e.voided=0  AND  e.location_id=:location  and
							o.voided=0 AND o.concept_id=5497 AND e.encounter_type =13 ) cd4
			GROUP BY patient_id
		) cd4 ON cd4.patient_id =  inicio_real.patient_id
       
	    /* ************ ******************************  Ultima mestruacao    ******************** ******************************/
        LEFT JOIN(  
        	SELECT 	e.patient_id,o.value_datetime ,  MAX(encounter_datetime)
			FROM	encounter e 
					INNER JOIN obs o ON o.encounter_id=e.encounter_id
			WHERE 	e.voided=0 AND 
					o.voided=0 AND o.concept_id=1465 AND e.encounter_type IN (6,9,53) 
                    AND e.form_id = 163 
			GROUP BY e.patient_id
        ...
        left join (
          ...
          /* ******************************** ultima carga viral *********** ******************************/
        LEFT JOIN(  
         SELECT ult_cv.patient_id, e.encounter_datetime , o.value_numeric as carga_viral 
			FROM	
        ...
        LEFT JOIN(  
            ...
            END  AS carga_viral_qualitativa,
				MAX(encounter_datetime) data_cv_qualitativa
                
			FROM encounter e 							
				INNER JOIN obs o ON o.encounter_id=e.encounter_id
			WHERE 	e.voided=0  AND e.location_id= :location  AND e.encounter_type in (6,9) and
					o.voided=0 AND 	o.concept_id=1305 and   o.location_id= :location
                    group by e.patient_id
...
GROUP BY visita2.patient_id  
	...
	LEFT JOIN (
     ...
     max(encounter_datetime)  as data_ult_esc
